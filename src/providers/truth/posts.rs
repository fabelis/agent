...
impl Client {
    pub async fn get_posts(&mut self) -> Result<Vec<Post>, anyhow::Error> {
        let mut stream = self.clone().create_tls_stream()?;

        let request = format!(
            "GET /api/v1/accounts/{}/statuses?exclude_replies=true&only_replies=false&with_muted=true HTTP/1.1\r\n\
