...
impl Client {
    ...
    pub fn agent(&self, model_name: &str) -> rig::agent::AgentBuilder<DeepSeekCompletionModel> {
        rig::agent::AgentBuilder::new(self.completion_model(model_name))
    }
}
...
impl CompletionModel for DeepSeekCompletionModel {
    ...
    async fn completion(
    ...
    ) -> Result<CompletionResponse<DeepSeekResponse>, rig::completion::CompletionError> {
        ...
        messages_json.push(json!({
            "role": "user",
            "content": if !request.documents.is_empty() {
                format!(
                    "<attachments>\n{}</attachments>\n\n{}",
                    request
                        .documents
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| rig::completion::CompletionError::RequestError(Box::new(e)))?;

        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(rig::completion::CompletionError::ProviderError(format!(
                "DeepSeek call failed: {status} - {text}"
            )));
        }

        let json_resp: DeepSeekResponse = resp.json().await.map_err(|e| {
        ...
        let model_choice = rig::completion::ModelChoice::Message(content);
